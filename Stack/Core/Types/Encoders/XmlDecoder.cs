        ...
        public XmlElement ReadXmlElement(string fieldName)
        {
            if (BeginField(fieldName, true))
            {
                if (MoveToElement(null))
                {
                    ...
                    value.NamespaceURI = m_reader.NamespaceURI;
                    value.LocalName = m_reader.LocalName;
                    value.Prefix = m_reader.Prefix;
                    value.Name = m_reader.Name;

                    StringBuilder builder = new StringBuilder();
                    ExtractXml(builder);
                    value.OuterXml = builder.ToString();
                    #else
                    XmlDocument document = new XmlDocument();
