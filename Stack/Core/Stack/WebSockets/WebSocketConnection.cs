...
namespace Opc.Ua.Bindings
{
    ...
    public sealed class WebSocketConnection : IDisposable
    {
        ...
        private async Task ProcessHttpUpgradeRequest(string message)
        {
            ...
            using (var writer = new StreamWriter(m_stream, new UTF8Encoding(false), UInt16.MaxValue, true))
            {
                ...
                if (origin != null)
                {
                    ...
                    await writer.WriteAsync(origin);
                    await writer.WriteAsync("\r\n");
                }
        ...
        private async Task<int> ReceiveAsync(byte[] buffer, int offset, int bytesToRead)
        {
            ...
            do
            {
                bytesRead = await m_stream.ReadAsync(buffer, offset, bytesToRead);
        ...
        private async Task<WebSocketFrame> ReceiveFrameAsync(Stream stream, byte[] buffer, int offset, int length)
        {
            ...
            result = await ReceiveAsync(buffer, pos, 2);
            ...
            {
                result = await ReceiveAsync(buffer, pos, 8);
            ...
            {
                mask = pos;
                result = await ReceiveAsync(buffer, pos, 4);
            ...
            if (payloadLength > 0)
            {
                ...
                result = await ReceiveAsync(buffer, pos, (int)payloadLength);
        ...
        public async Task<ArraySegment<byte>> ReceiveMessage()
        {
            ...
            var chunk = m_bufferManager.TakeBuffer(0, "WebSocketConnection.ReceiveFrameAsync");
            ...
            try
            {
                ...
                do
                {
                    if (State == ConnectionState.WaitingForHttpUpgrade)
                    {
                        if (m_isServerSide)
                        {
                            var request = await ReceiveHttpHeader();
                            await ProcessHttpUpgradeRequest(request);
                        }
                    }
