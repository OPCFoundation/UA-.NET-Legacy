...
namespace Opc.Ua.Bindings
{
    ...
    /// </summary>
    public class TlsTransportChannel : ITransportChannel
    {
        ...
        private TlsConnection m_connection;
        ...
        private const string g_ImplementationString = "TlsTransportChannel UA-{0} " + AssemblyVersionInfo.CurrentVersion;
        #endregion

        public TlsTransportChannel(ApplicationConfiguration configuration)
        {
        ...
        private void ProcessError(TlsConnection connection, ArraySegment<byte> chunk)
        {
            ...
            {
                m_bufferManager.ReturnBuffer(chunk.Array, "TlsTransportListener.ProcessAcknowledge");
                Utils.Trace(e, "ERROR [TlsTransportChannel.ProcessOpen] Could not process OpenSecureChannel response.");
        ...
        private void ProcessAcknowledge(TlsConnection connection, ArraySegment<byte> chunk)
        {
            ServiceResult result = null;
        ...
        private void ProcessOpen(TlsConnection connection, ArraySegment<byte> chunk)
        {
            ...
            {
                m_bufferManager.ReturnBuffer(chunk.Array, "TlsTransportListener.ProcessResponse");
                Utils.Trace(e, "ERROR [TlsTransportChannel.ProcessOpen] Could not process OpenSecureChannel response.");
                result = new ServiceResult(e);
            }
            ...
            if (ServiceResult.IsGood(result))
            {
                ...
                if (ar != null)
                {
                    ...
                    ar.Connection.Enqueue(message2);
                }
            }
            else
            {
        ...
        private void ProcessResponse(TlsConnection connection, ArraySegment<byte> chunk)
        {
            ...
            {
                m_bufferManager.ReturnBuffer(chunk.Array, "TlsTransportListener.ProcessResponse");
                Utils.Trace(exception, "ERROR [TlsTransportChannel.ProcessResponse] Could not process response.");
                return;
            }
            ...
            lock (m_requests)
            {
                ...
                {
                    Utils.Trace(Utils.TraceMasks.Information, "WARNING [TlsTransportChannel.ProcessResponse] Response received for unknown request.");
                    return;
                }
            }
        ...
        private async void ConnectAsync(SendRequestAsyncResult ar)
        {
            try
            {
                ...
                ar.Connection = await m_listener.ConnectAsync(
                    m_url.DnsSafeHost, 
                    IPAddress.Any, 
                    (m_url.Port <= 0) ? Utils.UaTlsDefaultPort : m_url.Port,
                    (m_url.Scheme == Utils.UriSchemeOpcTcp) ? TlsProtocol.None : TlsProtocol.Tls12,
                    ar.CancellationToken);

                m_connectInProgress = ar;
                var message = m_serializer.ConstructHelloMessage();
                ar.Connection.Enqueue(message);
            }
            catch (Exception e)
            {
                ar.Exception = e;
        ...
        private void SaveSettings(Uri url, TransportChannelSettings settings)
        {
            ...
            m_listener = new TlsListener(m_settings);
