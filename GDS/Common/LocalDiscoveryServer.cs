...
namespace Opc.Ua.Gds
{
    ...
    {
        #region Constructors
        public LocalDiscoveryServer(ApplicationConfiguration configuration)
        {
            ...
            string endpointTransportProfileUri)
        {
            ...
            if (!String.IsNullOrEmpty(endpointTransportProfileUri))
            {
                if (endpointTransportProfileUri == Profiles.WsHttpXmlTransport || endpointTransportProfileUri == Profiles.HttpsXmlTransport)
                {
                    configuration.UseBinaryEncoding = false;
                }
            }

            ITransportChannel channel = DiscoveryChannel.Create(ApplicationConfiguration, new Uri(endpointUrl), configuration, context);
